...
# now build a classifier for ENTIRE timecourse
clf = LogisticRegression(solver='lbfgs',C=1)
training_data_reshaped = np.reshape(training_data_classifier_SRM,(nSub,vox_num*nTR_new))
clf.fit(training_data_reshaped,trainingLabels)
filename = 'saved_classifiers' + '/' 'LOGISTIC_lbfgs_UPPERRIGHT_NOstations_' + '_' + 'ROI_' + str(maskType) + '_AVGREMOVE_' + str(removeAvg)  + '_filter_' + str(filterType) + '_k1_' + str(k1) + '_k2_' + str(k2)  + '.sav'
