...
nulllDist = np.int(sys.argv[2]) # if you want to draw from a null distribution
...
for sub in range(num_subs):
    prepared_data.append(images_concatenated[:, story_TR_1:story_TR_2,sub])
for sub in range(num_subs):
...
for f1 in np.arange(n_per_category):
	...
	for f2 in np.arange(n_per_category):
		...
		filename = 'new_group_SRM/aggregated_SRM_' + str(features) + 'f_removed_no_s1_' + str(f1) + '_no_s2_' + str(f2) + '.npy'
