...
for sub in range(n_per_category):
    print(trainingInd_BOOTSTRAP[sub])
...
trainingLabels = np.array([paranoidLabel[0:19],cheatingLabel[0:19] ]).flatten()
...
for stationInd in np.arange(nStations):
    ...
    filename = 'saved_classifiers' + '/' 'TRAIN_TEST_BOOTSTRAP_KEEP_ALL_UPPERRIGHT_stationInd_' + str(stationInd) + '_' + 'ROI_' + str(maskType) + '_AVGREMOVE_' + str(removeAvg)  + '_filter_' + str(filterType) + '_k1_' + str(k1) + '_k2_' + str(k2)  + '.sav'
...
save_str = 'new_bothphases/rtAtten_UPPER_RIGHT_winners_nofilter_predictproba_' + '_ROI_' + str(maskType) + '_AVGREMOVE_' + str(removeAvg)  + '_filter_' + str(filterType) + '_k1_' + str(k1) + '_k2_' + str(k2)  + '_perm_' + str(array_number) + '.npz'
