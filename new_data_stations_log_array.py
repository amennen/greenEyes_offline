# purpose: load satation dictionary and train separate spatiotemporal files
# script:
...
# don't need lowhigh bc will do stations
# will always do spatiotemporal stations

#lowhigh = np.int(sys.argv[2]) # whether to use low (1), high (2), or all (0)
#classifierType = np.int(sys.argv[3]) # whether you want (1) temporal or (2) spatiotemporal classification
#maskType = np.int(sys.argv[5]) # 0 = use DMN, 1 = TOM_large, 2 = TOM_cluster

logging.debug('loading in csv file to see which parameter values to use')
with open('array_combinations_specific.csv') as csv_file:
...
for ii in np.arange(n_iterations):
	...
	# for the classifier, keep all the training data in subjects x voxels x TRs to later separate at the last minute
...
# 9. Save accuracy
save_str = 'new_bothphases/STATIONDATA' + '_ROI_' + str(maskType) + '_AVGREMOVE_' + str(removeAvg)  + '_filter_' + str(filterType) + '_k1_' + str(k1) + '_k2_' + str(k2)  + '_perm_' + str(array_number) + '.npy'
