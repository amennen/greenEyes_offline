# purpose: to remove aggregate signal for each subject
...
for sub in range(num_subs):
    all_data.append(images_concatenated[:, :, sub])  #
for sub in range(num_subs):
...
prepared_data = []
for sub in range(num_subs):
    prepared_data.append(all_data[sub][:, story_TR_1:story_TR_2])  # #
...
for f1 in np.arange(n_per_category):
	...
	for f2 in np.arange(n_per_category):
		...
		filename = 'groupSRM/aggregated_SRM_' + str(features) + 'f_removed_no_s1_' + str(f1) + '_no_s2_' + str(f2) + '_storyTRz.npy'
