# PURPOSE: example testing of sklearn
...
if classifierType == 1:
    ...
    clf = SVC(kernel='linear', probability=True)
    filename = 'saved_classifiers' + '/' + 'TESTING_NEW_ROI_' + str(maskType) + '_AVGREMOVE_' + str(removeAvg) + '_classifierType_' + str(classifierType) + '_filter_' + str(filterType)  + '_k1_' + str(k1) + '_k2_' + str(k2)  + '.sav'
...
training_data_fn = 'training_data_testing_predict_proba.npy'
np.save(training_data_fn,training_data_classifier_SRM)
######## NOW TEST CLASSIFIER #######
# okay now go through each subject looking at 
# trianingLabels [1's and 0's]
# clf
# training_data_classifier_SRM: nSub x (nVox*nTR)
clf.predict(training_data_classifier_SRM): gives you just 1''s and 0''s in the correct order
clf.predict_proba(training_data_classifier_SRM)