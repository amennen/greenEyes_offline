...
classifierType = np.int(sys.argv[3])
lowhigh=0
filterType = 0
maskType = 1
	...
	for row in csv_reader:
		...
		if line_count > 0:
			...
			if i == array_number:
				k1=np.int(row[1])
				k2=np.int(row[2])
				removeAvg=np.int(row[3])
				C_param=np.float(row[4])
		line_count += 1
...
for ii in np.arange(n_iterations):
	...
	if classifierType == 1:
		...
		for t in np.arange(nTR_new):
			...
			clf = LogisticRegression(solver='lbfgs',C=C_param)
...
# 9. Save accuracy
save_str = 'logistic_test/LOGISTIC_ARRAYDATA_BS_no_replace' + '_ROI_' + str(maskType) + 'C_param' + str(C_param) + '_AVGREMOVE_' + str(removeAvg) + '_classifierType_' + str(classifierType) + '_filter_' + str(filterType) + '_lowhigh_' + str(lowhigh) + '_k1_' + str(k1) + '_k2_' + str(k2)  + '_perm_' + str(array_number) + '.npy'
