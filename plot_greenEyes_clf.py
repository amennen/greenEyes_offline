...
df.to_csv('thesis_plots/logistic_results.csv')
...
plt.subplot(3,1,1)
pl = sns.barplot(data=df_iter,x="rank",y="combo_acc",ci=95)
...
pl.text(i+.5, .95, 'AVG removed', horizontalalignment='left', size='small', color='black',weight='light')
pl.text(i+.5, 0.85, 'k1', horizontalalignment='left', size='small', color='black',weight='light')
...
plt.title('')
plt.xlim([-.5,n_options_show-1+.5])
plt.savefig('classifier_search_test_logistic.pdf')

plt.show()

# i don't know if average adds any information here so i'll just sort by average and show each
...
winning_index=118
winning_dict = {}
z = np.concatenate((all_accuracy_data_TR[:,winning_index],all_accuracy_data_ST[:,winning_index]),axis=0)
...
winning_dict['clf'] = ['TR']*n_iter + ['spatiotemporal'] * n_iter
df = pd.DataFrame.from_dict(data=winning_dict)
COLORS=['#969696','#bdc9e1', 'r'] # grey for TR, blue grey, red
